...
import { bedInputChangedActionCreator, feeInputChangedActionCreator, depositInputChangedActionCreator, movingInDateChangedActionCreator, nameInputChangedActionCreator, nextPaymentDateInputChangedActionCreator, passportPhotoInputChangedActionCreator, phoneInputChangedActionCreator, surnameInputChangedActionCreator, roomInputChangedActionCreator, updateTenant, fetchTenant, subscribeHistoryActionCreator, setSuccessUrlActionCreator } from '../../../redux/Reducers/editTenantReducer';
import styles from './EditTenant.module.sass';
...
export const EditTenant = ({state, dispatch}) =>{
	...
	const getRoomOptions = (rooms) => {
		return rooms?.map(room=>{
				if(state.initialData.room == room.url)
					return <option value={room.url} key={room.id} selected>{room.room_name}</option>
				else
					return <option value={room.url} key={room.id}>{room.room_name}</option>
			}
		)
	}
	...
	const roomSelectChanged = () =>{
		dispatch(roomInputChangedActionCreator(formRefs.room.current.value))
	}
	...
	const handleSubmit = event =>{
		...
		dispatch(setSuccessUrlActionCreator(`/room/?id=${state.data.room.match(/\d+/g).pop()}`))
		dispatch(updateTenant(state.data, state.initialData, tenantId ,headers))
	...
	return(
		<form onSubmit={handleSubmit} className={classes(styles.Form, styles.Form__editForm)}>
			...
			<div className={styles.FormFileInput}>
				...
				<img src={state.data.passportPhoto} ref={formRefs.passPhotoImage} className={styles.FormFileInput_FileImage} alt="-" />
			...
			<div className={styles.FormInput}>
				<select name="roomSelect" ref={formRefs.room} onChange={roomSelectChanged} className={styles.FormInput_select}>
					{getRoomOptions(state.adjacentRooms)}
				</select>
			</div>
